...
for (i in 1:num_simulations) {
  simulation_results <- readRDS(paste0("./Config_",num_config,"/A_Idsimulation_",i-1,".rds"))
  df_list[[i]] <- simulation_results
  simulation_results_joint <- readRDS(paste0("./Config_",num_config,"/A_Idsimulation_joint_",i-1,".rds"))
  df_list_joint[[i]] <- simulation_results_joint
  
}
results_proba <- bind_rows(df_list)
saveRDS(results_proba,paste0("./Config_",num_config,"/Final_Output_Id.rds"))
results_proba_joint <- bind_rows(df_list_joint)
saveRDS(results_proba_joint,paste0("./Config_",num_config,"/Final_Output_Id_joint.rds"))
