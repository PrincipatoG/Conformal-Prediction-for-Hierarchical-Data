...
### Compute the Coverage and Length from the simultaneous individual Conformal Prediction ###
conformal <- function(data_calib, data_test, n, m, method, alpha = 0.1){
  info_interval <- data.frame(métrique = c("Coverage", "Length", "Quantile"))
  for (i in 1:m){
    ...
    # The Length is the squared length (to correspond to the ojective)
...
### Compute the Coverage and Length from the Joint Conformal Prediction ###
root_det <- function(A) {
  m <- nrow(A)
...
joint_conformal <- function(data_calib, Sigma, data_test, n, m, method, alpha = 0.1){
  info_interval <- data.frame(métrique = c("Coverage", "Volume", "Quantile"))
    ...
    # q_Upper <- quantile(score,  (length(score)+1)*(1-alpha/2)/(length(score) ), type = 1) 
    # The Length is the squared length (to correspond to the ojective)
    # Volume <- (pi**(m/2))/ gamma(m/2+1)* sqrt(q_joint)**m *sqrt(det(Sigma))
    # C <- cbind(y_tilde_i_test + q_Lower, y_tilde_i_test + q_Upper)
...
### Compute the Coverage and Length from the Joint Conformal Prediction ###
joint_conformal_plot <- function(data_calib, Sigma, data_test, n, m, method, alpha = 0.1){
  info_interval <- data.frame(métrique = c("Coverage", "Volume", "Quantile"))
  y_calib <- data_calib[,paste0("V", 1:m)]
  y_test <- data_test[,paste0("V", 1:m)]
  if (method == "Direct") {
  ...
  new_col <- data.frame(V = c(Coverage, Volume, 1-alpha))
  info_interval <- info_interval %>% cbind(new_col)
  # saveRDS(Sigma, paste0("./Config_",num_config,"/Sigma_",simulation_indice,".rds"))
  # saveRDS(y_test, paste0("./Config_",num_config,"/Observations_",simulation_indice,".rds"))
  # saveRDS(y_tilde_test, paste0("./Config_",num_config,"/Forecasts_",method,"_",simulation_indice,".rds"))
  # saveRDS(sqrt(q_joint), paste0("./Config_",num_config,"/Radius_",method,"_",simulation_indice,".rds"))
  
  return(info_interval)
}
### Synthetize the metric from the function conformal ###
...
### Synthetize the metric from the function conformal and save the test data ###
metrics_proba_multi_plot <- function(data_calib, Sigma, data_test, n, m,  methods, alpha = 0.1) {
  data_list <- list()  
  joint_results <- list()
  for (method in methods) {
    print("First the simultaneous coverage")
    ...
    data_list[[method]] <- info_interval
    print("Then the joint coverage")
    joint_results[[method]] <- joint_conformal_plot(data_calib, Sigma, data_test, n, m, method, alpha = alpha)
  }
  print("By now everything should be fine")
  combined_data <- do.call(rbind, data_list)
  long_data <- combined_data %>%
...
simulation_multi <- function(S, n_obs, model){
  ...
  prediction_fast <- function(data, model = "gam", train, m){
    ...
    for (i in 1:m){
      ...
      # end.time <- Sys.time()
      # time.taken <- end.time - start.time
      # print(paste0("time to train (without learning the regularization) : "))
      # print(time.taken)
...
### Perform the simulation and save the test data ###
simulation_multi_plot <- function(S, n_obs, model, A = "Sigma"){
  # print(paste0("go for n= ",n))
  alpha <- 0.1
  # Split the available data
  train <- seq(1,floor((0.4)*n_obs))
  ...
  W_OLS <- diag(m)
  print("Compute the reconciliation matrices")
  error <- as.matrix(fl(data[valid,1:m])-(forecasts[valid,]))
  W_MinT <- cov(as.numeric(error))
  W_WLS <- diag(diag(W_MinT))
  G_MinT  <- ginv(t(S)%*%ginv(W_MinT)%*%S)%*%t(S)%*%ginv(W_MinT)
  P_MinT  <- S %*%G_MinT
  ...
  if(A=="Diag"){
    Sigma <- diag(diag(W_MinT))
  }
  result_conformal <- metrics_proba_multi_plot(data_calib, Sigma = Sigma, data_test, n, m, methods)
  return(result_conformal)
}
